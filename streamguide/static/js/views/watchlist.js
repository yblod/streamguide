...
export async function render(root, params) {
  ...
  const show = () => {
    ...
    else if (f.only === 'rent') list = list.filter((t) => !t.availability?.mine && (t.availability?.rent || []).length);  // Leihen (Kaufen nicht relevant)
  ...
  root.append(
    ...
    h('div', { class: 'glass panel row spread' },
      h('div', { class: 'row' },
        seg([['all', 'Alle'], ['available', 'Bei meinen Anbietern'], ['free', 'Kostenlos'], ['rent', 'Leihen'], ['none', 'Nicht verfügbar']], 'only')),
